...
Momentum from that gesture cannot advance another card or restart the landing. Once a card commits, another card unlocks only after the configured quiet gap, even if the event stream accelerates or reverses. Before commitment, a deliberate direction reversal can restart the gesture. Browser wheel events do not consistently expose actual finger release, so the quiet gap is a heuristic; the separate gesture-separation control is available for tuning on your trackpad.
